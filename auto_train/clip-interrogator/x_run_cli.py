...
def main():
    ...
    parser.add_argument('-f', '--folder', default='pic_folder',help='path to folder of images')
    parser.add_argument('-i', '--image',help='image file or url')
    parser.add_argument('-m', '--mode', default='best', help='best, classic, or fast')
    parser.add_argument("--lowvram", default='blip-base', action='store_true', help="Optimize settings for low VRAM")
    # model_path 
    parser.add_argument("--path", default='./hhgg')
    ...
    config = Config(device=device, clip_model_name=args.clip, clip_model_path="/work_disk/clip-interrogator/clip-vit-large-patch14")
    if args.lowvram:
    ...
    # process folder of images
        ...
        for file in files:
            ...
            prompt=  func(prompt)
            with open(os.path.join(args.folder, f"{file}.txt"), 'a', encoding='utf-8') as f:
                f.writelines(prompt)
        '''
