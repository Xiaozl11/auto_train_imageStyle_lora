...
file="$0"
pretrained_model="./sd-models/$1" # base model path | 底模路径
is_v2_model=0                             # SD2.0 model | SD2.0模型 2.0模型下 clip_skip 默认无效
parameterization=0                        # parameterization | 参数化 本参数需要和 V2 参数同步使用 实验性功能
train_data_dir="./train/pic"              # train dataset path | 训练数据集路径
...
save_every_n_epochs=$(($max_train_epoches/2)) # save every n epochs | 每 N 个 epoch 保存一次
...
output_name="$4"           # output model name | 模型保存名称
...
python3 -m accelerate.commands.launch ${launchArgs[@]} --num_cpu_threads_per_process=8 "./sd-scripts/train_network.py" \
  --enable_bucket \
