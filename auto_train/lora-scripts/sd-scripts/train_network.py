...
class NetworkTrainer:
    ...
    def train(self, args):
        ...
        accelerator.print(f"  num train images * repeats / 训练图像数×迭代次数: {train_dataset_group.num_train_images}")
        accelerator.print(f"  num reg images / 正则化图像的数量: {train_dataset_group.num_reg_images}")
        accelerator.print(f"  num batches per epoch / 1 epoch 的批次数: {len(train_dataloader)}")
        accelerator.print(f"  num epochs / epoch数: {num_train_epochs}")
        accelerator.print(
            f"  batch size per device /批: {', '.join([str(d.batch_size) for d in train_dataset_group.datasets])}"
        )
        # accelerator.print(f"  total train batch size (with parallel & distributed & accumulation) / 総バッチサイズ（並列学習、勾配合計含む）: {total_batch_size}")
        accelerator.print(f"  gradient accumulation steps / 对斜率求和的步数 = {args.gradient_accumulation_steps}")
        accelerator.print(f"  total optimization steps / 学习步骤数: {args.max_train_steps}")
